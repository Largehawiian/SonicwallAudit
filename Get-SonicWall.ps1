function Get-SonicWall {
    ...
    begin {
        ...
        switch ($Endpoint) {
            ...
            "AddressObjects" { Invoke-RestMethod "https://$($Script:Connection.PubIP):2020/api/sonicos/address-objects/ipv4" -Method 'GET' -Headers $headers}
            "Users-LoggedIn" { (Invoke-RestMethod "https://$($Script:Connection.PubIP):2020/api/sonicos/user/status/logged-in" -Method 'GET' -Headers $headers).replace("SSL VPN client",'"SSL VPN client"')| ConvertFrom-Json}
        }
