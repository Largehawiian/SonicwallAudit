...
function Get-Targets {
    ...
    if (!$TFA){
    $GetAccountQuery = "
    select * From accounts where MFAEnabled='N' AND Active='Y' AND Gen7='N'
    GO
    "
    if ($AccountName){ $GetAccountQuery = " SELECT * From Accounts WHERE AccountName LIKE '$($AccountName)%'"}
}
    else{
        $GetAccountQuery = "
        select * From accounts where AND Active='Y'
        GO
        " 
    }
    $sqloutput = Invoke-Sqlcmd -ServerInstance $instance -Database $DBName -Credential $creds -Query $GetAccountQuery -Debug -ErrorAction Inquire
    return $sqloutput
}
